...
class ProgramTrackerApp:
    ...
    def export_to_pdf(self, filename=None):
        ...
        try:
            ...
            for i, p in enumerate(self.participants):
                ...
                # Add programs
                if p.programs:
                    ax.text(27, i, ", ".join(p.programs), ha='left', va='center', fontsize=12, color='black')
    ...
    def show_assessment(self):
        ...
        def save_assessment():
            self.current_participant.red_phase_assessments[week] = {
                ...
                # 🔁 Find and update in main participants list
            for i, p in enumerate(self.participants):
                if p.name == self.current_participant.name and p.signup_date == self.current_participant.signup_date:
    ...
    def draw_progress_bar(self):
        ...
        for week in range(total_weeks):
            ...
            NEON_COLORS = {
                "red":    {"fill": "#E53935", "outline": "#FF6B6B"},     # Neon Red (iOS-style)
                "orange": {"fill": "#F4511E", "outline": "#FFB347"},     # Neon Orange
                "green":  {"fill": "#388E3C", "outline": "#34C759"}      # Neon Green
            }
            if color_key in NEON_COLORS:
                c = NEON_COLORS[color_key]
                fill = c["fill"]
                # White border on current week = "glow"
            ...
            # Optional: Add a tiny top highlight for "glossy" effect
            self.progress_canvas.create_line(x1 + 2, y1 + 2, x2 - 2, y1 + 2, fill="white", width=1)

            # Week number (white for contrast)
    ...
    def review_assessments(self):
        if not self.current_participant:
            messagebox.showinfo("Info", "No participant selected")
            return
       # No phase restriction — allow in all phases
        ...
        for week_num in range(1, 7):
            ...
            frame = ttk.LabelFrame(scrollable_frame, text=f"Week {week_num}", padding=10)
            ...
            # Notes
            if notes.strip():
                ttk.Label(frame, text="Notes:", font=(self.font_family, 9, "underline")).pack(anchor="w", pady=(5, 0))
                ...
                ttk.Label(frame, text="No notes provided.", foreground="#888", font=(self.font_family, 9)).pack(anchor="w")
    ...
    def apply_theme(self):
        ...
        # === Define brand colors ===
        bg_color = self.bg_color           # "#FFFFFF"
        fg_color = self.fg_color           # "#000000"
        primary = self.primary_color       # "#1A3A2A"
        accent = self.accent_color         # "#4CAF50"
        card_bg = self.card_bg             # "#FFFFFF"
        border = self.border_color         # "#1A3A2A"
        progress_bg = self.progress_bg     # "#F8F8F8"
        font = (self.font_family, 10)
...
if __name__ == "__main__":
    ...
    # Check password
    if password == "918273645":  # ← Change this!
        root.deiconify()  # Show main window
        try:
            print("Password correct, initializing app...")
            app = ProgramTrackerApp(root)
            print("App initialized successfully, starting main loop...")
            root.mainloop()
        except Exception as e:
